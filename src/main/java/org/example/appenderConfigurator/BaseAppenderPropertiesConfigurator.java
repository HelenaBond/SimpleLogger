...
public abstract class BaseAppenderPropertiesConfigurator implements AppenderPropertiesConfigurator {
    ...
    public void baseConfiguration(Appender appender, Properties properties, String prefix) {
        String messageFormat = properties.getProperty("%s.message.format".formatted(prefix));
        if (messageFormat != null) {
            appender.setMessageFormat(messageFormat);
        }
        String dateTimeFormat = properties.getProperty("%s.datetime.format".formatted(prefix));
        ...
        String levelFrom = properties.getProperty("%s.level.from".formatted(prefix));
        if (levelFrom != null) {
            try {
        ...
        String levelTo = properties.getProperty("%s.level.to".formatted(prefix));
        if (levelTo != null) {
            try {
